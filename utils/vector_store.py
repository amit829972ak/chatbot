...
def initialize_vector_store():
    ...
    try:
        ...
        # If there are no items in the database, add some sample knowledge
        if not items:
            sample_knowledge = [
        ...
        # If we still have no items (possibly due to database issues), create a minimal fallback index
        if not items:
            print("Warning: No knowledge items could be loaded. Creating fallback index.")
